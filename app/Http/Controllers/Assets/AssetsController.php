...
class AssetsController extends Controller
{
    ...
    /**
     * NIS2 relevance and scope on an asset are governed by its category (reached through the
     * model): when the category is flagged for the NIS2 inventory, its assets inherit both
     * "relevant" and the inventory scope and cannot override them on the asset form. The
     * per-asset service impact and notes stay editable. Returns [nis_relevant, nis_scope].
     */
    private function resolveAssetNisFromCategory($modelId, bool $submittedRelevant, ?string $submittedScope): array
    {
        $category = null;
        if ($modelId) {
            $model = AssetModel::find($modelId);
            $category = $model ? Category::find($model->category_id) : null;
        }

        if ($category && $category->nis_inventory_required) {
            return [true, $category->nis_inventory_scope];
        }

        return [$submittedRelevant, $submittedScope];
    }
    ...
    public function store(CreateMultipleAssetRequest $request): RedirectResponse
    {
        ...
        try {
            ...
            for ($a = 1, $aMax = count($asset_tags); $a <= $aMax; $a++) {
                ...
                [$asset->nis_relevant, $asset->nis_inventory_scope] = $this->resolveAssetNisFromCategory(
                    $request->input('model_id'), $request->boolean('nis_relevant'), $request->input('nis_inventory_scope')
                );
    ...
    private function syncTenantServices(Request $request, Asset $asset): void
    {
        ...
        $validIds = TenantService::validIdsForCompany($ids, $asset->company_id);
